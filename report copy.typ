...
*Conclusions:* Intermediate neutrality maximises population diversity through a balance between neutral exploration and selective maintenance of population structure. Excessive neutrality ($p > 0.8$) reduces diversity as genetic drift overwhelms weak selection and collapses independent lineages. Together, these results support neutral network theory and are consistent with edge-of-chaos predictions for the neutrality–selection balance.
  ],
)  
...
While previous studies have demonstrated that neutrality can enhance evolvability, this study provides a quantitative demonstration that genetic diversity itself is maximised at intermediate neutrality due to lineage-level effects. In particular, we show that excessive neutrality reduces diversity through drift-driven lineage collapse, even when raw neutral mutation rates are high.
...
*Critical methodological note:* These metrics measure *raw genetic diversity* across the entire 50-bit genome, treating all loci equally regardless of their functional status. This is essential for understanding how neutrality affects population structure. At *p* = 0.75, for example, approximately 25% of sites (12-13 loci) remain functional with distinct contribution values, while 75% of sites (37-38 loci) evolve neutrally due to copied contribution values. Our diversity metrics make no distinction between these categories—we count differences at all 50 positions equally. Thus, when we observe that *p* = 0.75 maintains higher diversity than *p* = 0.9 (Hamming distance: 2.622 vs 2.376), this reflects genome-wide genetic variation, not just variation at functional sites. The key insight is that selection operating on a minority of sites (25%) can maintain diversity across the entire genome through linkage, whereas selection on only 10% of sites (*p* = 0.9) proves insufficient to prevent drift-driven homogenisation genome-wide.
...
For each neutrality level, we computed mean ± standard error across replicates for final diversity values. We compared diversity rankings between short (500 generation) and long (1500 generation) experiments to assess equilibrium convergence.
...
The diversity peak at intermediate neutrality (*p* = 0.75) was robust across metrics, rejecting both the null hypothesis of monotonic increase and the alternative of maximum diversity at extreme neutrality.
...
This resulted in a *reversal of ranking*, with *p* = 0.75 emerging as the clear optimum only after extended evolution. The dramatic difference in diversity growth rates suggests that *p* = 0.75 maintains ongoing exploration while *p* = 0.9 experiences stagnation.




#figure(
  image("time_panel_1.pdf", width: 100%),
  caption: [
    *Diversity trajectories reveal equilibrium-dependent ranking.* 
    Hamming distance over 1500 generations for four key neutrality levels. The *p* = 0.75 trajectory (orange) overtakes *p* = 0.9 (yellow) after approximately 500 generations (red vertical line), demonstrating that optimal neutrality emerges only at equilibrium. Low neutrality (*p* = 0, grey) shows rapid convergence to low diversity, whilst intermediate (*p* = 0.45, blue) exhibits intermediate dynamics. Shaded regions indicate standard error across 15 replicates. This temporal pattern highlights the necessity of extended evolutionary experiments to capture true equilibrium states.
  ]
)

#figure(
  image("time_panel_2.pdf", width: 100%),
  caption: [
    *Genetic entropy dynamics mirror Hamming distance patterns.* 
    Shannon entropy trajectories over 1500 generations show qualitatively similar temporal dynamics as Hamming distance. The *p* = 0.75 condition (orange) emerges as optimal at equilibrium, whilst *p* = 0.9 (yellow) shows early advantages that fade over time. This consistency across independent diversity metrics validates the inverted-U relationship between neutrality and population diversity. Shaded regions indicate standard error across 15 replicates.
  ]
)
...
== Selection Strength and Population Structure

Within-population fitness variance (a proxy for selection strength) decreased with neutrality:

- *p* = 0.0: $sigma_f$ = 0.0151 (*strongest selection*)
- *p* = 0.75: $sigma_f$ = 0.0109
- *p* = 0.9: $sigma_f$ = 0.0110 (*weakest selection*)

Despite similar selection strength at *p* = 0.75 and *p* = 0.9, diversity was markedly higher at *p* = 0.75. This suggests that the absolute magnitude of selection matters less than the proportion of genome under selection (25% vs 10% functional loci).

Despite similar instantaneous fitness variance at p = 0.75 and p = 0.9, selection at p = 0.75 was more effective in practice at maintaining lineage separation, whereas at p = 0.9 drift dominated population structure.

= 4 Discussion

== Neutrality-Selection Balance and the Diversity Optimum
Our results demonstrate that intermediate neutrality (p ≈ 0.75) maximises population genetic diversity through a balance between neutral exploration and selective maintenance of population structure. This inverted-U relationship supports theoretical predictions from neutral network theory @Wagner2008NeutralismReconciliation and edge-of-chaos models @Kauffman1993OriginsEvolution.
...
At *p* = 0.9, weak selection on only 10% of sites (5 loci) fails to prevent lineage mixing. Even though 90% of sites (45 loci) evolve neutrally, they lose diversity through drift in the absence of selective population structure. This demonstrates that selection indirectly maintains diversity at neutral sites—a principle central to molecular evolution theory @Kimura1983NeutralEvolution. The critical threshold appears to lie between *p* = 0.75 and *p* = 0.9, where selection becomes too weak to sustain the population structure necessary for diversity preservation.
...
*2. Asexual reproduction:* Recombination would alter neutral network connectivity and potentially shift the diversity optimum.
